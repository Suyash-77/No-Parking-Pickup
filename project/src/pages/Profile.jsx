...
const Profile = () => {
    ...
    const checkFaceRegistered = async () => {
        try {
            const { data } = await api.get('/api/user/face-status')
            if (data.success) setFaceRegistered(data.registered)
        } catch { }
    }
    ...
    try {
        await loadModels()
        setFaceStatus('Models loaded. Opening camera...')
        const s = await navigator.mediaDevices.getUserMedia({ video: true })
        setFaceStream(s)
        setFaceStatus('Camera opened. Attaching to video...')
        setTimeout(() => {
            if (faceVideoRef.current) {
                faceVideoRef.current.srcObject = s
                setFaceStatus('Position your face clearly and click Save Face')
            } else {
                setFaceStatus('Video element not found.')
            }
        }, 100)
    } catch (err) {
        // show exact error instead of generic message
        setFaceStatus(`Error: ${err.name} — ${err.message}`)
    ...
    const saveFace = async () => {
        ...
        try {
            const { data } = await api.post('/api/user/save-face', {
            ...
            if (data.success) {
                ...
                setFaceStatus(data.message || 'Failed to save face.')
            }
        } catch {
            setFaceStatus('Failed to save face. Try again.')
        }
    }

    const cancelFaceRegister = () => {
        faceStream?.getTracks().forEach(t => t.stop())
        setFaceRegistering(false)
        setFaceStatus('')
    }
    ...
    const isAdmin = userData?.role === USER_ROLE.DASHBOARD_ADMIN || userData?.role === USER_ROLE.FIELD_ADMIN
                ...
                <div style={{ width: '100%', maxWidth: '900px' }}>
                        ...
                        {!profileEdit ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxWidth: '500px' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    ...
                                    <input className="search-input" value={profileForm.name}
                                        onChange={e => setProfileForm(f => ({ ...f, name: e.target.value }))}
                                        placeholder="Full name" />
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    <label style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Email</label>
                                    <input className="search-input" value={userData?.email || ''} disabled
                                        style={{ opacity: 0.5, cursor: 'not-allowed' }} />
                                </div>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    <label style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Phone</label>
                                    <input className="search-input" value={profileForm.phone}
                                ...
                                <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
                                    ...
                                    <button className="btn-reject" onClick={handleProfileCancel}>Cancel</button>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="admin-card" style={{ marginBottom: '24px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            ...
                            <button className="btn-approve" onClick={() => navigate('/add-vehicle')}>
                                + Add Vehicle
                            </button>
                        </div>
                                            ...
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', maxWidth: '600px' }}>
                                                {[
                                                    ...
                                                    <div key={key} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                        ...
                                                        <input className="search-input" value={vehicleForm[key]}
                                                ...
                                                <div style={{ gridColumn: 'span 2', display: 'flex', gap: '10px', marginTop: '4px' }}>
                                                    ...
                                                    <button className="btn-reject" onClick={handleVehicleCancel}>Cancel</button>
                                                </div>
                                            </div>
                                        )}
                                    </>
                    ...
                    {/* Face Login Card — admins only */}
                    {isAdmin && (
                        <div className="admin-card">
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                                <h2 style={{ margin: 0 }}>Face Login</h2>
                                {faceRegistered && (
                            ...
                            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '16px' }}>
                                Register your face to skip password login next time.
                            </p>

                            {faceStatus && (
                                <div className={faceStatus.includes('successfully') ? 'alert-success' : 'alert-error'}
                            ...
                            <div style={{ display: 'flex', gap: '10px' }}>
                                {!faceRegistering ? (
                                    <button className="btn-approve" onClick={startFaceRegister}>
                                        {faceRegistered ? 'Re-register Face' : 'Register Face'}
                                    </button>
                                ) : (
                                    <>
                                        <button className="btn-approve" onClick={saveFace}>Save Face</button>
                                        <button className="btn-reject" onClick={cancelFaceRegister}>Cancel</button>
                                    </>
                                )}
                            </div>
                        </div>
